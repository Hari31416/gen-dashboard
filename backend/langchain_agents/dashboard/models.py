...
class ChartGoal(BaseModel):
    ...
    aggregation: AggregationType = Field(
        default=AggregationType.NONE, description="Aggregation to apply"
    )

    # Filters
    filters: Optional[Dict[str, Any]] = Field(
        default=None, description="Any filters to apply"
    )
...
class StrategyAgentOutput(BaseModel):
    ...
    chart_goals: List[ChartGoal] = Field(
        ..., description="List of 3-5 chart objectives"
    )
...
class DataAgentOutput(BaseModel):
    ...
    chart_data_results: List[ChartDataResult] = Field(
        ..., description="Data for all charts"
    )
    total_execution_time_ms: float = Field(
        ..., description="Total time for all queries"
    )
...
class VegaLiteEncoding(BaseModel):
    ...
    type: str = Field(
        ..., description="Data type: quantitative, nominal, ordinal, temporal"
    )
...
class SingleVizSpec(BaseModel):
    ...
    selection: Optional[Dict[str, Any]] = Field(
        None, description="Vega-Lite selection definitions"
    )
...
class VizSpecAgentOutput(BaseModel):
    ...
    viz_specs: List[SingleVizSpec] = Field(
        ..., description="Vega-Lite specs for all charts"
    )
...
class LayoutConfig(BaseModel):
    ...
    cols: int = Field(
        ...
        ge=1,
        le=24,
        description="Total grid columns (default 12 for react-grid-layout)",
    )
    row_height: int = Field(100, description="Height of each row in pixels")
    layout: List[ChartLayoutPosition] = Field(
        default_factory=list, description="Position of each chart"
    )
...
class ComposedDashboardSpec(BaseModel):
    ...
    vega_lite_spec: Dict[str, Any] = Field(
        default_factory=dict, description="Complete Vega-Lite JSON"
    )

    # NEW: Individual chart specs for flexible layout
    individual_specs: List[Dict[str, Any]] = Field(
        default_factory=list, description="Individual Vega-Lite specs for each chart"
    )

    # NEW: Grid layout configuration
    layout_config: Optional[LayoutConfig] = Field(
        None, description="react-grid-layout compatible layout configuration"
    )

    # Layout info
    layout_type: LayoutType = Field(
        default=LayoutType.GRID, description="Layout arrangement used"
    )
    chart_count: int = Field(..., description="Number of charts in dashboard")

    # Metadata for refresh/refine
    sql_queries: List[Dict[str, str]] = Field(
        default_factory=list, description="SQL queries by chart_id for refresh"
    )
...
class DashboardGenerateRequest(BaseModel):
    ...
    max_charts: int = Field(
        default=5, ge=1, le=10, description="Maximum charts to generate"
    )
...
class DashboardRefineRequest(BaseModel):
    ...
    new_feedback: Optional[str] = Field(
        None, description="Text feedback for refinement"
    )
    filter_state: Optional[Dict[str, Any]] = Field(
        None, description="New filter values"
    )
...
class DashboardResponse(BaseModel):
    ...
    # Dashboard data
    dashboard: Optional[ComposedDashboardSpec] = Field(
        None, description="Generated dashboard"
    )
...
# =============================================================================
# Smart Refinement Models
# =============================================================================


class RefinementActionType(str, Enum):
    """Supported refinement action types."""

    RERUN_SQL = "rerun_sql"  # Re-execute existing SQL (refresh data)
    MODIFY_SQL = "modify_sql"  # Fix/change SQL query (LLM regenerates)
    CHANGE_CHART_TYPE = "change_chart_type"  # Switch visualization type
    CHANGE_ENCODING = "change_encoding"  # Change x/y fields, colors, grouping
    CHANGE_TITLE = "change_title"  # Update chart or dashboard title
    CHANGE_LAYOUT = "change_layout"  # Rearrange/resize charts
    ADD_CHART = "add_chart"  # Add new chart to dashboard
    REMOVE_CHART = "remove_chart"  # Remove existing chart
    CHANGE_THEME = "change_theme"  # Change color scheme/styling
    FULL_REDESIGN = "full_redesign"  # Complete dashboard overhaul


class RefinementAction(BaseModel):
    """
    A single refinement action to perform on the dashboard.

    Generated by the Intent Classifier Agent based on user feedback.
    """

    action_type: RefinementActionType = Field(
        ..., description="Type of refinement action"
    )
    target_chart_id: Optional[str] = Field(
        None, description="Chart ID to target. None means all charts or dashboard-level"
    )
    parameters: Dict[str, Any] = Field(
        default_factory=dict,
        description="Action-specific parameters (e.g., new_type='line', new_title='Sales')",
    )
    confidence: float = Field(
        default=1.0,
        ge=0.0,
        le=1.0,
        description="Confidence score for this action classification",
    )


class RefinementIntent(BaseModel):
    """
    Complete output from the Intent Classifier Agent.

    Contains one or more refinement actions derived from user feedback.
    """

    actions: List[RefinementAction] = Field(
        default_factory=list, description="List of refinement actions to perform"
    )
    requires_clarification: bool = Field(
        default=False,
        description="True if user intent is ambiguous and needs clarification",
    )
    clarification_question: Optional[str] = Field(
        None, description="Question to ask user if clarification is needed"
    )
    reasoning: str = Field(
        default="", description="Explanation of how the intent was classified"
    )


class DashboardFilterRequest(BaseModel):
    """Request model for /api/dashboard/filter endpoint (fast filter-only updates)."""

    session_id: str = Field(..., description="Session ID from previous generation")
    filter_state: Dict[str, Any] = Field(..., description="Filter values to apply")


class RefinementClarificationResponse(BaseModel):
    """Response when refinement needs user clarification."""

    requires_clarification: bool = Field(True)
    clarification_question: str = Field(..., description="Question to ask the user")
    suggested_actions: List[str] = Field(
        default_factory=list, description="Suggested refinement options"
    )
